...
Module Main
    Sub Main()
        int totalNodeNum = CInt(2 ^ (maxDepth + 1) - 1)
        double dimension[][] = Double(numberofData - 1)(){}
        For i = 0 To numberofData - 1 
            dimension(i) = New Double(featureDim - 1) {}
            For j = 0 To featureDim - 1
                dimension(i)(j) = 0 'Assign everything as 0; this will hold the featurn values of each data, each label
            Next
        Next
        'Label array
        Dim label() = New Integer(numberofData - 1){} 'nth data' s label / class
        ...
        'Learning
        For treeindex = 0 To numberTrees - 1
            'initial
            'Saves where data is saved, in which node
            Dim arrival(numberofData - 1) As Integer
            'Nodes stated array 0: missing node, 1: Split node, 2: Leaf node
            Dim nodelists = New Integer(totalNodeNum - 1){}
            'Leaf node info
            Dim LeafStatistic()() = New Double(totalNodeNum - 1)() {}
            'every leaf node, label proportion
            'Split node info
            Dim SplitStatistic()() = New Double(totalNodeNum - 1)(){}
            'node parameters
            For i = 0 To totalNodeNum - 1
            ...
            For i = 0 To totalNodeNum - 1
                SplitStatistic(i) = New Double(1){} 'feature and its threshold
            Next

            For i = 0 To totalNodeNum - 1
                LeafStatistic(i) = New Double(numberofLabel - 1){}
            Next
    ...
    Function loadthedatafile(dimension As Double()(), label As Integer()) As Double()()
        ...
        For i = 0 To numberofData - 1
            ...
            If minvalofLabel > templabel Then
                minvalofLabel = templabel
            End If
            If maxvalofLabel < templabel Then
                maxvalofLabel = templabel
            End If
        Next
        ...
        For i = 0 To numberofData - 1
        '*** is it possible to combine those two For loops into a single For loops
            'Console.WriteLine("Reading data in the " & i + 1 & "-th Line")
            line = filereader.ReadLine
